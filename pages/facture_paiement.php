...
<div class="container p-5 mt-4 rounded shadow-lg">
    ...
    <p>Le total de la facture est de <span class="fw-bold"><?php echo formaterPrix($facture->totalTTC())?></span></p>

    <div class="border rounded mt-3 p-3">
        <h4>Choississez le moyen de paiement :</h4>

        <p>Il est possible de payer avec les différents points disponibles au client</p>
        ...
        <div class="row">
            ...
            <div class="col">
                ...
                if(count($points) > 0) {
                    echo '<select class="form-select" size="'. count($points) .'">';
                    foreach ($points as $i => $point) {
                        echo '<option value="'. $i .'">' . $point->intitule . ' : ' . formaterPrix($point->quantite * $valeur_point) . '</option>';
                    }
                ?>
                </select>
                <?php
